...
binds to `127.0.0.1:8000` only (no public surface). Reach it either by:

- an SSH tunnel: `ssh -L 8000:127.0.0.1:8000 you@your-host` then open
  `http://localhost:8000`, or
- your own reverse proxy / TLS in front of `127.0.0.1:8000` (set
...
2. Set `SENTRY_DSN=<that DSN>` in the app's environment and restart the app.
   (For the localhost setup the DSN host will be `localhost:8000`; the app
