    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-4 md:p-6">
          ...
          {/* Areas (still under Space tab) */}
          {activeTab === 'space' && (
            <div className="bg-card rounded border border-border p-6 mt-6">
              <p className="font-mono text-[10px] tracking-widest text-muted-foreground uppercase mb-1">Areas</p>
              <p className="font-mono text-[10px] text-muted-foreground mb-6">
                Areas appear in the Tasks, Projects, and KB dropdowns. Defaults are seeded when a space
                is created. Rename, reorder, or archive freely; existing tasks tagged with the old
                name will not auto-rename.
              </p>

              <ul className="divide-y divide-border mb-6">
                {areas.length === 0 ? (
                  <li className="py-3 font-sans text-sm text-muted-foreground">No areas yet.</li>
                ) : (
                  areas.map(area => (
                    <li key={area.id} className="py-2 flex items-center gap-3">
                      <input
                        type="number"
                        value={area.sort_order}
                        onChange={async e => {
                          const next = parseInt(e.target.value || '0', 10)
                          setAreas(prev => prev.map(a => (a.id === area.id ? { ...a, sort_order: next } : a)))
                        }}
                        onBlur={async e => {
                          if (!isAdmin) return
                          const next = parseInt(e.target.value || '0', 10)
                          await updateArea({ areaId: area.id, sort_order: next })
                        }}
                        disabled={!isAdmin}
                        className="w-14 bg-background border border-border rounded px-2 py-1 font-mono text-xs text-foreground disabled:opacity-50 focus:outline-none focus:border-primary"
                      ...
                      <input
                        ...
                        onChange={e => {
                          ...
                          setAreas(prev => prev.map(a => (a.id === area.id ? { ...a, name: next } : a)))
                        }}
                        onBlur={async e => {
                          if (!isAdmin) return
                          const next = e.target.value.trim()
                          if (!next || next === area.name) return
                          const result = await updateArea({ areaId: area.id, name: next })
                          if ('error' in result && result.error) {
                            setMessage({ type: 'error', text: result.error })
                          }
                        }}
                        disabled={!isAdmin}
                        maxLength={80}
                        className={`flex-1 bg-background border border-border rounded px-3 py-1.5 font-sans text-sm rounded ${
                          area.is_archived ? 'text-muted-foreground line-through' : 'text-foreground'
                        } disabled:opacity-50 focus:outline-none focus:border-primary`}
                      />
                      <span className="font-mono text-[10px] text-muted-foreground/60 w-24 truncate" title={area.code}>
                        {area.code}
                      </span>
                      {isAdmin && (
                        <>
                          <button
                            type="button"
                            onClick={async () => {
                              const result = await updateArea({ areaId: area.id, is_archived: !area.is_archived })
                              if ('error' in result && result.error) {
                                setMessage({ type: 'error', text: result.error })
                              } else {
                                setAreas(prev =>
                                  prev.map(a =>
                                    a.id === area.id ? { ...a, is_archived: !a.is_archived } : a,
                                  ),
                                )
                              }
                            }}
                            className="font-mono text-[10px] tracking-widest uppercase border border-border text-muted-foreground hover:text-foreground rounded px-2 py-1"
                          >
                          ...
                          <button
                            type="button"
                            onClick={async () => {
                              if (!confirm(`Delete area "${area.name}" entirely? Existing tagged items keep the name as plain text. Use Archive instead if it has been used.`)) return
                            ...
                            className="font-mono text-[10px] tracking-widest uppercase border border-border text-muted-foreground hover:text-red-500 hover:border-red-500 rounded px-2 py-1"
                          >
                            delete
                          </button>
                        </>
              ...
              {isAdmin && (
                <form
                  onSubmit={async e => {
                    e.preventDefault()
                    const code = newArea.code.trim().toLowerCase()
                    const name = newArea.name.trim()
                    if (!code || !name) return
                    setSaving(true)
                    const result = await createArea({ code, name })
                    if ('error' in result && result.error) {
                      setMessage({ type: 'error', text: result.error })
                    } else if ('data' in result && result.data) {
                      setAreas(prev =>
                        [...prev, result.data as Area].sort(
                  ...
                  className="border-t border-border pt-4 grid grid-cols-1 sm:grid-cols-[1fr_2fr_auto] gap-3 items-end"
                >
                  <div>
                    ...
                    <input
                      ...
                      maxLength={40}
                      placeholder="leather"
                      className="w-full bg-background border border-border rounded px-3 py-2 font-mono text-sm text-foreground focus:outline-none focus:border-primary"
                    />
                  </div>
                  <div>
                    <label className="font-mono text-[10px] tracking-widest text-muted-foreground uppercase block mb-1">
                  ...
                  <button
                    type="submit"
                    disabled={saving || !newArea.code.trim() || !newArea.name.trim()}
