...
### UX polish — Tier 1 COMPLETE (S1-S8 deployed; S3 `108c2a5` local/unpushed); Tier 2/3 next
...
- **Tier 2 (next, not started):** founder first-run guided dashboard when the space is empty (`app/(app)/dashboard/page.tsx`); onboarding Skills controlled value + required-ack hint (`app/onboarding/onboarding-flow.tsx`).
- **Tier 3 (after Tier 2):** secrets copy + auto re-hide + clear-on-unmount; permissions matrix read-only notice + per-checkbox aria-label; OpsAclEditor `aria-pressed`/check/toast. Presentational-only, must stay inside the RLS guardrail (no change to reveal action, `user_has_permission`, `ops_acl`, self-change trigger).
- Sequence: finish Tier 2 + Tier 3, deploy, THEN the forms feature.
